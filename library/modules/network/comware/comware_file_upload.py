...
module: comware_file_upload
short_description: Upload a local file to a remote Comware v7 device
description:
    - Upload a local file to a remote Comware v7 device
version_added: 1.8
category: System (RW)
notes:
...
# copy file
- comware_file_upload: file=/usr/smallfile remote_path=flash:/otherfile username={{ username }} password={{ password }} hostname={{ inventory_hostname }}
...
try:
    ...
    from pyhpecw7.features.file_upload import FileUpload
...
def main():
    ...
    try:
        file_upload = FileUpload(device, proto, src, dst)
        if not file_upload.remote_dir_exists:
            file_upload.create_remote_dir()
        file_upload.transfer_file()
        changed = True
    ...
    results = {}
    results['source_file'] = file_upload.src
    results['destination_file'] = file_upload.dst
